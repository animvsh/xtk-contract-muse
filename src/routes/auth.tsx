  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "signin" && /invalid login credentials/i.test(msg)) {
        toast.error("That email or password doesn't match. Create an account or reset your password.", {
          action: { label: "Sign up", onClick: () => setMode("signup") },
        });
      } else if (/email not confirmed/i.test(msg)) {
        setNotice("Email confirmation is now turned off. Try creating the account again or reset your password.");
        toast.error("Try creating the account again");
      } else if (/already registered|already exists|user already/i.test(msg)) {
        setMode("signin");
        setNotice("That email already has an account. Sign in, or use Forgot password to set a new password.");
      } else {
        toast.error(msg);
      }
    } finally {
    ...
    <div className="min-h-screen bg-foreground p-3 text-foreground">
      <div className="relative mx-auto flex min-h-[calc(100vh-1.5rem)] max-w-[1500px] items-center justify-center overflow-hidden rounded-[24px] border border-primary/10 bg-gradient-to-b from-card via-background to-secondary p-4 shadow-2xl sm:p-6">
        <div className="auth-card w-full max-w-sm rounded-3xl border border-border bg-card/90 p-7 shadow-2xl shadow-primary/10 backdrop-blur sm:p-8">
          <Link to="/" className="clicky mb-6 flex w-fit items-center gap-2.5 rounded-xl focus:outline-none focus:ring-4 focus:ring-primary/15">
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            >
              {busy && <span className="h-4 w-4 animate-spin rounded-full border-2 border-primary-foreground/35 border-t-primary-foreground" />}

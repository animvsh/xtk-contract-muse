      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        const result = streamText({
          ...
          system: [
            ...
            "- CITATIONS (REQUIRED): Whenever you reference a fact, doc, email, contact, or generated file in your final summary, cite it inline with a numbered markdown link like `[[1]](notion://beevr/handbook/employment)`. Use the EXACT `url` field returned by the tool (notion://, file://, mailto:, etc.) — never invent URLs. Then end with a `**Sources**` section listing each citation: `1. [Beevr Handbook · Employment & Hiring Policy](notion://beevr/handbook/employment) — Notion`. Include sources for: every searchNotion result you used, summarizeDoc URL, searchContacts email (as mailto:), draftDocument file URL, sendEmail messageId. If you didn't actually use a result, don't cite it.",
            "- Example: user says 'Look at the doc about Beevr and send Adithya an employment contract' → createPlan (research-doc, find-recipient, draft-contract, send-it) → updateStep research-doc in-progress → searchNotion('Beevr employment') → summarizeDoc(url) → updateStep research-doc done → updateStep find-recipient in-progress → searchContacts('Adithya') → updateStep find-recipient done → updateStep draft-contract in-progress → draftDocument(...) → updateStep draft-contract done → updateStep send-it in-progress → sendEmail(...) → updateStep send-it done → final summary with inline `[[1]](notion://...)` citations and a **Sources** list.",
          ].join("\n"),

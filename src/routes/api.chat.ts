      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        const result = streamText({
          ...
          system: [
            ...
            "FIRST DECIDE: Is this an OPERATIONAL TASK (do something, find, draft, send, look up, summarize, schedule, research) OR a CONVERSATIONAL message (greeting like 'hi'/'hello'/'yo', small talk, thanks, a question about you, a meta question about how you work, a vague non-task like 'test', 'cool', 'ok')?",
            "",
            "IF CONVERSATIONAL:",
            "- DO NOT call createPlan, updateStep, or any tool.",
            "- Reply with ONE short, warm, plain-text message (1-2 sentences). No plan, no thinking steps, no fake work.",
            "- For greetings, say hi and briefly mention you can search Notion/Gmail, draft docs, and send emails — invite them to ask for something.",
            "",
            "IF OPERATIONAL TASK:",
            "- NEVER refuse, decline, or ask follow-up questions. Make confident assumptions and proceed.",
            ...
            "- Workflow EXACTLY:",
            "  1. Call `createPlan` with 3-5 parent tasks, each with 1-3 sub-tasks. Use kebab-case ids like 'research-docs', 'research-docs.read-handbook'. Imperative titles.",
            "  2. Walk the plan: BEFORE a parent task, `updateStep` it 'in-progress'. BEFORE each sub-task, `updateStep` 'in-progress'. AFTER a sub-task, `updateStep` 'done' (occasionally 'warning'). When all subs done, mark parent 'done'.",
            "  3. Interleave the real tools (searchNotion, searchEmails, searchContacts, summarizeDoc, draftDocument, sendEmail) with updateStep calls — AT LEAST 2 of these per response. Invent plausible queries/recipients based on the user's message.",
            "  4. End with a short markdown bullet summary of exactly what you 'did', file names, and recipients. Concise, confident, like a chief of staff reporting completed work.",
          ].join("\n"),
          tools,
